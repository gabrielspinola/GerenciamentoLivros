...
from model.LivrosAlugadosModel import LivrosAlugadosModel, LivrosAlugadosModelVazio
...
class LivrosAlugadosRoute(Routes):
    ...
    def register_routes(self):        
        ...
        def alugar_Livro():
            ...
            livroAlugado_service = LivrosAlugadosServices(username=session.get('username'), password=session.get('usuario_pass'))
            livroAlugado_service.create(livroAlugado)
            
            # Bloquear o livro alugado
            livro_services = LivroServices(username=session.get('username'), password=session.get('usuario_pass'))
        ...
        def listar_LivrosAlugados():
            livroAlugado = LivrosAlugadosServices(username=session.get('username'), password=session.get('usuario_pass'))
        ...
        @Routes.login_required
        def novoAluguel():
            livroAlugado_data = LivrosAlugadosModelVazio()
            listaLivros_disponiveis = LivroServices(username=session.get('username'), password=session.get('usuario_pass')).listar_livros_disponiveis()
            listaUsuarios = UsuarioServices(username=session.get('username'), password=session.get('usuario_pass')).listar_allAtivos()
        ...
        def consultar_LivroAlugado(id):
            ...
            #LivroServices(username=session.get('username'), password=session.get('usuario_pass')).DesbloqueiaLivroID(altLivroAlugado.idlivro)
        ...
        def atualizar_LivroAlugado(id):
            livroAlugado_Data = LivrosAlugadosServices(username=session.get('username'), password=session.get('usuario_pass')).consultar_id(id)
            listaLivros_disponiveis = LivroServices(username=session.get('username'), password=session.get('usuario_pass')).listar_all()
            listaUsuarios = UsuarioServices(username=session.get('username'), password=session.get('usuario_pass')).listar_allAtivos()
        ...
        def deletar_LivroAlugado(id):
            ...
            # Desbloquear o livro alugado
            LivroServices(username=session.get('username'), password=session.get('usuario_pass')).DesbloqueiaLivroID(aluguel.idlivro)
